...
static auto load_script(entt::registry& reg, const std::string& path, sol::environment& env)
...
auto load_module(entt::registry& reg, const std::string& path) -> entt::resource<LuaAsset> {
    return AssetManager::load<LuaAsset>(reg, path);
}

LuaAsset::LuaAsset(std::string path, entt::registry& reg) :
    state(reg.ctx().get<std::shared_ptr<sol::state>>()),
    path(path),
    env(*state, sol::create, state->globals()) {
    auto result = load_script(reg, path, env);
    if (!result) {
        error_state = true;
        return;
    }
...
auto LuaAsset::load(const std::string& path, entt::registry& reg) -> LuaAsset {
    return {path, reg};
}
...
auto LuaAsset::reload(this LuaAsset& self, const std::string&, entt::registry& reg) -> void {
    auto new_env = sol::environment(*self.state, sol::create, self.state->globals());

    auto result = load_script(reg, self.path, new_env);
    if (!result) return;
