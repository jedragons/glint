...
auto NameComp::on_construct(entt::registry& registry, entt::entity e) -> void {
    const auto& comp = registry.get<NameComp>(e);
    registry.ctx().get<NameMap>().set(comp.name(), e);
}

auto NameComp::on_update(entt::registry& registry, entt::entity e) -> void {
    const auto& comp = registry.get<NameComp>(e);
    registry.ctx().get<NameMap>().rename(e, comp.name());
}

auto NameComp::on_destroy(entt::registry& registry, entt::entity e) -> void {
    const auto& comp = registry.get<NameComp>(e);
    registry.ctx().get<NameMap>().remove(comp.name());
}
