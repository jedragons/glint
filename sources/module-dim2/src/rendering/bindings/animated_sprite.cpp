...
auto bind_animated_sprite(entt::registry& registry) -> void {
    ...
    lua.new_usertype<AnimatedSpriteComp>(
         ...
         */
        "load",
        [&registry](const std::string& atlas, const std::string& sprite) -> AnimatedSpriteComp {
            return {load_atlas(registry, atlas)->animation(sprite)};
        },
