...
static auto bind_globals(entt::registry& registry, sol::state_view lua) -> void;
static auto bind_flags(entt::registry& registry, sol::state_view lua) -> void;
static auto bind_ui(entt::registry& registry, sol::state_view lua) -> void;
static auto bind_window(entt::registry& registry, sol::state_view lua) -> void;
static auto bind_canvas(entt::registry& registry, sol::state_view lua) -> void;

auto bind_ui(entt::registry& registry) -> void {
    auto lua = core::get_lua(registry);
    bind_globals(registry, lua);
    bind_flags(registry, lua);
    bind_ui(registry, lua);
    bind_window(registry, lua);
    bind_canvas(registry, lua);
}
...
static auto bind_globals(entt::registry& registry, sol::state_view lua) -> void {
    /*@
      ...
      function ui() end
     */
    lua.globals()["ui"] = [&registry]() -> nk_context& { return *registry.ctx().get<nk_context*>(); };
}

static auto bind_flags(entt::registry&, sol::state_view lua) -> void {
    lua.new_enum(
        /*@
          ---@class UiFlagsTable
...
static auto bind_ui(entt::registry&, sol::state_view lua) -> void {
    lua.new_usertype<nk_context>(
         ...
         */
        "window",
        [](nk_context& ctx, const std::string& title, Rectangle bounds, unsigned int flags, sol::function func)
            -> void {
...
static auto bind_window(entt::registry& registry, sol::state_view lua) -> void {
    lua.new_usertype<nk_window>(
        ...
        "bounds",
        sol::property([&registry](nk_window& w) -> Rectangle {
            return NuklearRectToRectangle(get_nuklear(registry), w.bounds);
        }),
...
static auto image_with_region(Texture2D texture, Rectangle bounds) {
    auto img = TextureToNuklearImage(texture);
    img.region[0] = static_cast<nk_ushort>(bounds.x);
    img.region[1] = static_cast<nk_ushort>(bounds.y);
    img.region[2] = static_cast<nk_ushort>(bounds.width);
    img.region[3] = static_cast<nk_ushort>(bounds.height);
    return img;
}
...
static auto bind_canvas(entt::registry& registry, sol::state_view lua) -> void {
    lua.new_usertype<nk_command_buffer>(
         ...
         */
        "stroke_line",
        [](nk_command_buffer& b, float x0, float y0, float x1, float y1, float line_thickness, Color color) -> void {
            nk_stroke_line(&b, x0, y0, x1, y1, line_thickness, ColorToNuklearColor(color));
        },
         ...
         */
        "stroke_rect",
        [&registry](nk_command_buffer& b, Rectangle rect, float rounding, float line_thickness, Color color) -> void {
            const auto nk_rect = RectangleToNuklearRect(get_nuklear(registry), rect);
            const auto col = ColorToNuklearColor(color);
         ...
         */
        "stroke_circle",
        [&registry](nk_command_buffer& b, Rectangle rect, float line_thickness, Color color) -> void {
            const auto nk_rect = RectangleToNuklearRect(get_nuklear(registry), rect);
            const auto col = ColorToNuklearColor(color);
         ...
         */
        "stroke_polyline",
        [](nk_command_buffer& b, sol::table points, float line_thickness, Color color) -> void {
            const auto col = ColorToNuklearColor(color);
         ...
         */
        "fill_rect",
        [&registry](nk_command_buffer& b, Rectangle rect, float rounding, Color color) -> void {
            const auto nk_rect = RectangleToNuklearRect(get_nuklear(registry), rect);
            const auto col = ColorToNuklearColor(color);
         ...
         */
        "fill_circle",
        [&registry](nk_command_buffer& b, Rectangle rect, Color color) -> void {
            const auto nk_rect = RectangleToNuklearRect(get_nuklear(registry), rect);
            const auto col = ColorToNuklearColor(color);
         ...
         */
        "fill_arc",
        [](nk_command_buffer& b, float cx, float cy, float radius, float a_min, float a_max, Color color) -> void {
            const auto col = ColorToNuklearColor(color);
            const auto a_min_rad = a_min * DEG2RAD;
            const auto a_max_rad = a_max * DEG2RAD;
         ...
         */
        "fill_triangle",
        [](nk_command_buffer& b, float x0, float y0, float x1, float y1, float x2, float y2, Color color) -> void {
            const auto col = ColorToNuklearColor(color);
         ...
         */
        "fill_polygon",
        [](nk_command_buffer& b, sol::table points, Color color) -> void {
            const auto col = ColorToNuklearColor(color);
            thread_local auto point_vec = std::vector<float> {};
            const auto count = static_cast<int>(points.size());
         ...
         */
        "draw_image",
        [&registry](nk_command_buffer& b, Rectangle rect, dim2::SpriteComp sprite, Color color) -> void {
            auto img = image_with_region(sprite.sprite->texture->texture(), sprite.sprite->frame.rect);
            const auto nk_rect = RectangleToNuklearRect(get_nuklear(registry), rect);
