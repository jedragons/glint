...
AtlasAsset::AtlasAsset(const std::string& path, entt::registry& reg) {
    auto& vfs = core::get_vfs(reg);

    const auto texture_path = std::filesystem::path(path).replace_extension("png").string();
    auto texture = core::AssetManager::load<core::TextureAsset>(reg, texture_path);
    ...
    for (const auto& [name, frame] : meta.frames) {
        frames.insert({
            ...
            std::make_shared<Sprite>(Sprite {
                .texture = texture,
                .frame = SpriteFrame {
                    .rect = raylib::Rectangle(float(frame.x), float(frame.y), float(frame.w), float(frame.h)),
...
auto AtlasAsset::load(const std::string& path, entt::registry& reg) -> AtlasAsset {
    return {path, reg};
}

auto AtlasAsset::reload(this AtlasAsset& self, const std::string& path, entt::registry& reg) -> void {
    auto new_atlas = AtlasAsset {path, reg};
...
auto setup_atlases(entt::registry& reg) -> void {
    core::AssetManager::register_asset<AtlasAsset>(reg);
}

auto load_atlas(entt::registry& reg, const std::string& path) -> entt::resource<AtlasAsset> {
    return core::AssetManager::load<AtlasAsset>(reg, path);
}
